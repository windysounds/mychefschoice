document.addEventListener('DOMContentLoaded', () => {
    ...
    // ★★★ 뷰 전환 함수 수정: 'blog' 와 나머지 정적 페이지를 pageContent에서 함께 처리 ★★★
    ...
    // ★★★ 언어 설정 함수 수정: 현재 뷰를 더 안정적으로 다시 그리도록 개선 ★★★
    function setLanguage(lang) {
        ...
        if (!activePage || ['home', 'result'].includes(activePage)) return;
        ...
        if (activePage === 'blog') {
            ...
            displayStaticPage(activePage, lang);
        }
    }

    // ★★★ 블로그 및 정적 페이지를 그리는 새로운 함수들 ★★★
    function displayBlogPosts(lang) {
        pageContent.dataset.pageType = 'blog-list';
        pageContent.dataset.pageId = '';
        const blogHTML = `
            <div class="blog-section">
                <h2 id="blog-title">${translations.staticPages[lang].blogTitle}</h2>
        ...
        pageContent.innerHTML = blogHTML;
    }
