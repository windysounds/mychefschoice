document.addEventListener('DOMContentLoaded', () => {
    ...
    const translations = {
        ...
        staticPages: {
            ko: {
                ...
                about: `<h1>My Chef의 선택을 소개합니다</h1><p>...</p>`, 
                privacy: `<h1>개인정보 처리방침</h1><p>...</p>`,
                contact: `<h1>문의하기</h1><p>...</p>`
            },
            en: {
                ...
                about: `<h1>About My Chef's Choice</h1><p>...</p>`,
                privacy: `<h1>Privacy Policy</h1><p>...</p>`,
                contact: `<h1>Contact Us</h1><p>...</p>`
            },
            blog: [
                {
    ...
    function switchView(viewName, dataId = null) {
        ...
        if (viewName === 'home') {
            ...
            pageContent.dataset.pageId = viewName;
            pageContent.innerHTML = translations.staticPages[currentLang][viewName];
            pageContent.classList.remove('hidden');
        }
    }
    ...
    function setLanguage(lang) {
        ...
        if(activeBtn) {
            const activePage = activeBtn.dataset.page;
            if(activePage === 'blog') {
                displayBlogPosts(lang);
            } else if (pageContent.dataset.pageType === 'blog-post') {
                displayFullBlogPost(pageContent.dataset.pageId, lang);
            } else if (['about', 'privacy', 'contact'].includes(activePage)) {
                 pageContent.innerHTML = translations.staticPages[currentLang][activePage];
            }
        }
    }
    ...
    function displayBlogPosts(lang) {
        ...
        document.getElementById('blog-title').textContent = translations.staticPages[lang].blogTitle;
        container.innerHTML = translations.staticPages.blog.map(post => `
        ...
        if (post) {
            ...
            pageContent.innerHTML = `
                <div class="blog-post-content">
                    ...
                    <div class="blog-body">${post['contents_html_' + lang]}</div>
                </div>`;
        }
    }
    ...
    document.getElementById('lang-switcher').addEventListener('click', (e) => { if (e.target.tagName === 'BUTTON') setLanguage(e.target.dataset.lang); });

    navButtons.forEach(btn => {
        btn.addEventListener('click', (e) => {
            navButtons.forEach(b => b.classList.remove('active'));
            e.target.classList.add('active');
            switchView(e.target.dataset.page);
        });
    });
